...
3. Check the delivery address fields and informational note on Step 2.
4. Check that no card detail fields (Cardholder Name, Card Number, Expiry Date, CVV) are present.
5. Check the submit button label.

### Expected Result

1. The "🅿️ PayPal" option becomes the selected payment method.
2. The modal advances to Step 2 (URL remains `/cart`). The modal heading reads "🅿️ PayPal". An amount display is visible formatted as "Amount to pay: **LKR X.XX**".
3. The form contains exactly three delivery address fields: Street Address (placeholder: "No. 12, Main Street"), City (placeholder: "Colombo"), Phone (placeholder: "+94 77 000 0000"). The informational note reads: "You will be redirected to PayPal to complete your payment securely."
4. **No** card fields are present (Cardholder Name, Card Number, Expiry Date, CVV are absent).
5. The submit button label reads "Proceed to PayPal". A "← Back" button and a "←" header button are both visible.
